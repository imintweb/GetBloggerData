...
var _app = document.getElementById("mint-sitemap"),
    _load = document.querySelector(".mint-sitemap__loading"),
...
function manageData(json){
  json.items.forEach(function(element, index){
    var url = element.url.substring(0, element.url.indexOf("/", 8) + 1),
        ...
        li.className = "mint-sitemap__list-element";
    ...
    if(date[0] + date[1] == lastDate){
      ...
      _div = document.createElement("div");
      _div.className = "mint-sitemap";
      _div.innerHTML = "<h2 class='mint-sitemap__title'><a class='mint-sitemap__title-url' target='_blank' href='" + url + date[0] + "_" + date[1] + "_01_archive.html'>" + _months[Number(date[1]) - 1] + " " + date[0] + "</a></h2>";
      _ul = document.createElement("ul");
      _ul.className = "mint-sitemap__list";
      _ul.appendChild(li);
      lastDate = date[0] + date[1];
    }
